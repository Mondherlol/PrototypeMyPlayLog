...
exports.getUserGames=async (req, res)=> {
    ...
    const userID = await (await psn.getProfileFromUserName(authorization, username)).profile.accountId;
    
    //get all games 
    const response = await psn.getUserTitles(authorization, userID);
    var games = [...new Set(response.trophyTitles.map(({trophyTitleName}) => trophyTitleName))];
